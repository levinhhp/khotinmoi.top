...
<thead class="thead-dark" style="background:#343a40;">
    ...
    $config['base_url'] = site_url('adminhp/viewCheck/'.$table_name.'/'.$status);
    $config['uri_segment']=5;
    $config['total_rows'] = $rowCounter;
    $config['per_page'] = $rowLimit;
    $start= (int)$this->uri->segment(5) * $config['per_page']+1;
    $end = ($this->uri->segment(5) == floor($config['total_rows']/ $config['per_page']))? $config['total_rows'] : (int)$this->uri->segment(3) * $config['per_page'] + $config['per_page'];
...
<?php
    ...
    foreach($contents as $content)
    {
        ...
        {
            echo '<td align="center"><a title="Đổi mật khẩu" href="'.site_url().'adminhp/resetmatkhau/'.$content->$id.'"><img src="backend/img/icon-48-user.png" title="Đổi mật khẩu" alt="Đổi mật khẩu"></a></td>';        
        }
        elseif($table_name == 'tblarticle'){
                ...
                {				
                    ...
                    foreach($fields as $field)
                    {
                        ...
                        if($field->Field==$fieldName[$k])
                        {
                            ...
                            {
                                if(strlen($content->$check_default)>100)
                                {
    								...
    								echo '<div class="hidden_text">'.$content->$check_default.'</div>';
    								break;
                                }
                                else
                                {
...
<div class="col-lg-6">
    <br>
    <?php echo "Showing ".$start." - ".$end." of ".$config['total_rows']." Results"; ?>
</div>
