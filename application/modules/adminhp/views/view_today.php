...
<thead class="thead-dark" style="background:#343a40;">
    ...
    $config['base_url'] = site_url('adminhp/newToday/'.$table_name);
    $config['uri_segment']=4;
    $config['total_rows'] = $rowCounter;
    $config['per_page'] = $rowLimit;
    $start= (int)$this->uri->segment(4) * $config['per_page']+1;
    $end = ($this->uri->segment(4) == floor($config['total_rows']/ $config['per_page']))? $config['total_rows'] : (int)$this->uri->segment(3) * $config['per_page'] + $config['per_page'];
